...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/customer/checkcustomerlogin', { email, password, role });
      const user = res.data;
      const backendRole = user.role || role || 'user';
      localStorage.setItem('user', JSON.stringify(user));
      if (backendRole === 'doctor') navigate('/dashboard/doctor');
      else if (backendRole === 'management' || backendRole === 'admin') navigate('/dashboard/management');
